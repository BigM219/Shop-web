  ...
  return (
    <header className=" px-4 w-full h-14 sticky top-0 flex items-center bg-gray-50 shadow-md">
      <div className=" w-full flex justify-between items-center">
        <Link to="/">
          <div className="flex">
            <span className="text-[33px] text-yellow-950">
              <AiOutlineBug />
            </span>
            <span className="font-semibold text-2xl hidden sm:block">
              Dnuts<span className="text-yellow-950"></span>
            </span>
          </div>
        </Link>
        ...
        <button
          onClick={openSidebar}
          type="button"
          className={`inline-flex items-center p-1 text-sm text-gray-500 rounded-lg sm:hidden hover:bg-gray-100 focus:outline-none ring-2 ring-gray-300 focus:ring-yellow-950 ${
            sidebarMenu ? "focus:ring-red-700 ring-red-700" : ""
          }`}
        >
          <span className="sr-only">Open sidebar</span>
          {sidebarMenu ? (
            <span className="w-6 h-6 flex items-center justify-center text-2xl text-yellow-950">
              &#10005;
            </span>
          ) : (
            <svg
